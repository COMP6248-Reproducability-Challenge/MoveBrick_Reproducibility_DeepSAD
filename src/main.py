...
# known_outlier_class这个是已知异常类的类别号，每次进行新实验都需要更换，从0到9选
# known_outlier_class参数传进去以后仅当n_known_outlier_classes等于1时生效
# n_known_outlier_classes这个是已知异常类别的个数，即实验参数Kl, 
# 当n_known_outlier_classes大于1时（比如第三个实验表的后三列），known_outlier_class参数传什么值进去都不起作用
...
def main(dataset_name, net_name, xp_path, data_path, load_config, load_model, eta,
    ...
    for i in range(20):
        ...
        cfg.settings['seed'] = i + 1
        # normal_class -> [0,9]
        # 设置正常类
        normal_class = int(np.ceil(float(i/2) + 0.1)) - 1
        # known_outlier_class -> [0,9]
        outlier_classes = list(range(0, 10))
        outlier_classes.remove(normal_class)
        # 设置已知异常类
        known_outlier_class = random.sample(outlier_classes, 1)[0]
